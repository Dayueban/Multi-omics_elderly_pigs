...
# 所以绘制下venn图
a <- read.csv("Output_data/Feces_KO_tidy_kruskal_test20230302.csv",
              row.names = 1, header = T, check.names = F)
b <- read.csv("Output_data/Saliva_KO_tidy_kruskal_test20230303.csv",
              row.names = 1, header = T, check.names = F)
...
venn.diagram(
  ...
  filename = "Plots/venn_KO_intersect_feces_saliva.png",
  output = TRUE ,
  imagetype="png" ,
  height = 1200 , 
  width = 1200 , 
...
# 分别对feces和saliva交集的13个KO取出来作柱状图
...
ggsave("Plots/Feces_Saliva_intersect_in_feces_Ko.png",plot = p, width = 3, height = 6, dpi = 300)
...
##p <- p + geom_jitter() ##添加数据点
ggsave("Plots/Feces_Saliva_intersect_in_saliva_Ko.png",plot = p, width = 3, height = 6, dpi = 300)
