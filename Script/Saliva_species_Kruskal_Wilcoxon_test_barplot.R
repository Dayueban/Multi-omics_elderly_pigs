...
#######################################################################
# (1) 读取数据, 种水平的相对丰度表
a <- read.csv("saliva_metag/Saliva.Unigenes.absolute.s.csv", header = T, row.names = 1, check.names = F)
...
write.csv(a3, file="Output_data/Saliva_species_tidy_kruskal_test20230228.csv")
a3 <- read.csv("Output_data/Saliva_species_tidy_kruskal_test20230228.csv", header = T, row.names = 1,
               check.names = F)

# 1) 选择差异最大的前30个进行展示
a4 <- a3[order(a3$fdr.kw, decreasing = FALSE), ] # 根据fdr值进行排序
Taxo_df1 <- a4[1:30, 1:45]
Taxo_df1 <- Taxo_df1[order(Taxo_df1$OaSa15, decreasing = T), ]
...
ggsave("Plots/Saliva_species_tidy_top30_kw20230228.png",plot = p, width = 8, height = 16, dpi = 300)
...
a3 <- cbind(a2,Pvalue_ad_ma,Pvalue_ad_oa, Pvalue_ma_oa, fdr.ad_ma,
            fdr.ad_oa, fdr.ma_oa)
write.csv(a3, file="Output_data/Saliva_species_tidy_wilcoxn_test_BH.csv")

a3 <- read.csv("Output_data/Saliva_species_tidy_wilcoxn_test_BH.csv", header = T, row.names = 1, check.names = F)

# plot01 Ad and Ma
a4 <- a3[order(a3$fdr.ad_ma, decreasing = FALSE), ] # Ad vs Ma
Taxo_df1 <- a4[1:30, 1:30] # 选择差异最显著的前30个进行展示
Taxo_df1 <- Taxo_df1[order(Taxo_df1$MaSa15, decreasing = T), ]
rownames(Taxo_df1) <- str_split(rownames(Taxo_df1), ";", simplify = TRUE)[,2] # 去掉属信息，只保留种
Taxo_df2 <- Taxo_df1
Taxo_df3 <- as.data.frame(t(Taxo_df2))
...
p <- p + scale_y_continuous(trans = "sqrt", expand = c(0,0))
##p <- p + geom_jitter() ##添加数据点
ggsave("Plots/Saliva_species_tidy_Ad_Ma_top30_wilcoxn20230228.png",plot = p, width = 12, height = 8, dpi = 300)

# plot02 Ad and Oa
a4 <- a3[order(a3$fdr.ad_oa, decreasing = FALSE), ] # Ad vs Oa
Taxo_df1 <- a4[1:30, c(1:15, 31:45)] # 选择差异最显著的前30个进行展示
Taxo_df1 <- Taxo_df1[order(Taxo_df1$OaSa15, decreasing = T), ]
rownames(Taxo_df1) <- str_split(rownames(Taxo_df1), ";", simplify = TRUE)[,2] # 去掉属信息，只保留种
Taxo_df2 <- Taxo_df1
Taxo_df3 <- as.data.frame(t(Taxo_df2))
...
p <- p + scale_y_continuous(trans = "sqrt", expand = c(0, 0))
##p <- p + geom_jitter() ##添加数据点
ggsave("Plots/Saliva_species_tidy_Ad_Oa_top30_wilcoxn202302283.png",plot = p, width = 12, height = 8, dpi = 300)

# plot03 Ma and Oa
a4 <- a3[order(a3$fdr.ma_oa, decreasing = FALSE), ] # Ma vs Oa
Taxo_df1 <- a4[1:8, 16:45] # 选择差异最显著的前30个进行展示
Taxo_df1 <- Taxo_df1[order(Taxo_df1$OaSa15, decreasing = T), ]
rownames(Taxo_df1) <- str_split(rownames(Taxo_df1), ";", simplify = TRUE)[,2] # 去掉属信息，只保留种
Taxo_df2 <- Taxo_df1
Taxo_df3 <- as.data.frame(t(Taxo_df2))
...
p <- p + scale_y_continuous(trans = "sqrt", expand = c(0, 0))
##p <- p + geom_jitter() ##添加数据点
ggsave("Plots/Saliva_species_tidy_Ma_Oa_top30_wilcoxn20230228.png",plot = p, width = 6, height = 8, dpi = 300)
