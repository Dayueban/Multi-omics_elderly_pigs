...
#######################################################################
# (1) 读取数据, 种水平的相对丰度表
a <- read.csv("stool_metag/Unigenes.absolute.ko.csv", header = T, row.names = 1, check.names = F)
b <- read.csv("stool_metag/Unigenes.absolute.ko_with_attri.csv", header = T, row.names = 1, check.names = F)
# (2)数据处理
...
write.csv(a3, file="Output_data/Feces_KO_tidy_kruskal_test20230302.csv")

# 1) 选择差异最大的前50个进行展示
a4 <- a3[order(a3$fdr.kw, decreasing = FALSE), ] # 根据fdr值进行排序
#length(which(a3$fdr.kw < 0.05)) # 99
...
save_pheatmap_png(fecPhase_pheatmap, "stool_metag/stool_KO_all_kw_pheatmap20220302.png")
