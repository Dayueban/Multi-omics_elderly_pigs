...
a0.05 <- a[which(a$fdr.ad_oa < 0.05), c(1:15,31:45)]
a0.05t <- as.data.frame(t(a0.05))

dat <- a0.05t


# 计算差异的菌和差异的KO之间的相关性以及作图
KO <- read.csv("Output_data/Feces_KO_tidy_wilcoxn_test_BH.csv", header = T,
               row.names = 1, check.names = F)
KO0.05 <- KO[which(KO$fdr.ad_oa < 0.05), c(1:15, 31:45)]
KO_t <- as.data.frame(t(KO0.05))

# 要保证两组数据的个体ID是一样的
#dat2 <- dat[c(1:10, 16:25),]
library(plyr)
library(psych)
library(reshape2)
corr_df <- corr.test(dat, KO_t, method = "spearman", adjust = "BH", alpha = .05)
...
colnames(asso_df) <- paste(colnames(asso_df),rep(c("cor","P_adj"),c(36,36)),sep = "_")
...
pb <- ggplot(nbar.m0.6, aes(row, variable)) +
  ...
  labs(title="Microbial KOs associated with Microbial species") + 
...
ggsave("Microbial_species_assoc_KO_Oa_Ad.png", plot = pp, width = 14,height = 24, dpi = 300)
