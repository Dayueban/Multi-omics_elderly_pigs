...
#######################################################################
# (1) 读取数据, 种水平的相对丰度表
a <- read.csv("saliva_metag/Unigenes.absolute.level3.csv", header = T, row.names = 1, check.names = F)
b <- read.csv("saliva_metag/Unigenes.absolute.level3_with_attri.csv", header = T, row.names = 1, check.names = F)
# (2)数据处理
...
write.csv(a3, file="Output_data/Saliva_level3_tidy_kruskal_test20230303.csv")

# 1) 选择差异最大的前35个进行展示(也就35个)
a4 <- a3[order(a3$fdr.kw, decreasing = FALSE), ] # 根据fdr值进行排序
#length(which(a3$fdr.kw < 0.05)) # 46
...
save_pheatmap_png(fecPhase_pheatmap, "saliva_metag/saliva_level3_all_kw_pheatmap20220303.png")
