...
KO <- read.csv("Output_data/Feces_KO_tidy_wilcoxn_test_BH.csv", header = T,
                   row.names = 1, check.names = F)
KO0.05 <- KO[which(KO$fdr.ad_ma < 0.05), 1:30]
...
nbar.m0.7 <- nbar.m0.7[-(which(nbar.m0.7$variable == "Others")), ]
...
ggsave("Microbial_species_assoc_KO_Ma_Ad.png", plot = pp, width = 16,height = 24, dpi = 300)
