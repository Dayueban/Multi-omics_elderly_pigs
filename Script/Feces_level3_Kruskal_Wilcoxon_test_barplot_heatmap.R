...
b <- read.csv("stool_metag/Unigenes.absolute.level3_with_attri.csv", header = T, row.names = 1, check.names = F)
# (2)数据处理
...
# remove kegg pathway that called "others" from further analysis
a2 <- as.data.frame(a2[-which(rownames(a2) == "Others"),])
...
write.csv(a3, file="Output_data/Feces_level3_tidy_kruskal_test.csv")

# 1) 选择差异最大的前50个进行展示
a4 <- a3[order(a3$fdr.kw, decreasing = FALSE), ] # 根据fdr值进行排序
#length(which(a3$fdr.kw < 0.05)) # 70
...
Taxo_df22<- cbind(rownames(Taxo_df2), rowMeans(Taxo_df2[,1:15]), rowMeans(Taxo_df2[,16:30]), rowMeans(Taxo_df2[,31:45]))
...
save_pheatmap_png(fecPhase_pheatmap, "stool_metag/stool_level3_all_kw_pheatmap20220302.png")
